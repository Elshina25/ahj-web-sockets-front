export default class Chat {
    constructor(url, container) {
        ...
        this.ws.addEventListener('message', (e) => {
            ...
            if (response.type === 'error') {
                ...
                notice.classList.add('notivication');
                notice.textContent = response.data;
                this.chatWindow.appendChild(notice);
        ...
        window.addEventListener('beforeunload', () => {
            this.ws.send(JSON.stringify({ type: 'deleteUser', username: this.currentUser }));
        });
    ...
    createChatWindow() {
        ...
        chatForm.addEventListener('submit', (e) => {
            ...
            const timestamp = `${new Date().toLocaleDateString()} ${new Date().toLocaleTimeString().slice(0, -3)}`;
